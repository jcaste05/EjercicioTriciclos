...
'''
Programa que toma como entrada ficheros de distintos grafos e imprime por
pantalla todos los triciclos de cada uno de ellos
'''
...
def halla_triciclos(grafo_txt): #Toma como entrada un rdd que son las líneas del fichero de un grafo. Devuelve el rdd con los triciclos
...
def main(sc, ficheros):
    lista_grafos_txt_rdd = [] #Lista de rdd's
    for f in ficheros:
        fichero_rdd = sc.textFile(f)
        lista_grafos_txt_rdd.append(fichero_rdd)
    i = 1
    for rdd in lista_grafos_txt_rdd: #Hallamos los triciclos de cada grafo e imprimimos por pantalla el resultado
        aux = halla_triciclos(rdd)
        print(f'Triciclos del grafo {i}:')
        print(list(aux.collect()))
        i = i + 1
